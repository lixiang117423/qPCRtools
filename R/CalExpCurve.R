...
#' @param curve_table A data frame with standard curve parameters per gene.
...
CalExpCurve <- function(cq_table,
  ...
  # statistical tests
  df <- cal_curve_stat_test(df, stat_method, ref_group)
  ...
  p <- build_curve_plot(df.plot, fig_type, fig_ncol)
...
#' @keywords internal
cal_curve_stat_test <- function(df, stat_method, ref_group) {
  if (stat_method == "t.test") {
    df %>%
      dplyr::group_by(Gene) %>%
      rstatix::t_test(expre ~ Treatment, ref.group = ref_group) %>%
    ...
    df %>%
      dplyr::group_by(Gene) %>%
      rstatix::wilcox_test(expre ~ Treatment, ref.group = ref_group) %>%
      dplyr::select(Gene, group2, p) %>%
      dplyr::mutate(temp = paste0(Gene, group2)) %>%
      dplyr::select(temp, p) %>%
      dplyr::mutate(signif = dplyr::case_when(
...
#' @keywords internal
build_curve_plot <- function(df.plot, fig_type, fig_ncol) {
  if (fig_type == "box") {
    df.plot %>%
