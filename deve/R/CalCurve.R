...
#'
#' @importFrom dplyr select,rename,group_by,mutate,ungroup,select,rename
#' @importFrom stats lm
...
#' df.1 <- data.table::fread("inst/examples/calsc.cq.txt")
#' df.2 <- data.table::fread("inst/examples/calsc.info.txt")
#' CalCurve(cq.table = df.1,
#'          concen.table = df.2,
#'          lowest.concen = 4,
#'          highest.concen = 4096,
#'          dilu = 4,
